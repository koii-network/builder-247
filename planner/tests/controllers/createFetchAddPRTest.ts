...
async function test() {
  ...
  // const response = await axios.get("http://localhost:3000/api/create-to-do?title=test&acceptanceCriteria=test&repoOwner=test&repoName=test");
  ...
  try {
    const response2 = await axios.get(
      `http://localhost:3000/api/fetch-to-do?signature=${signature}&pubKey=${pubKey}&github_username=${githubUsername}`,
    );
  ...
  const githubPullRequestUrl =
    "https://github.com/koii-network/koii-core/pull/1234";
  console.log("1111111");
  try {
    const response3 = await axios.get(
      `http://localhost:3000/api/add-pr-to-to-do?signature=${signature3}&pubKey=${pubKey}&githubPullRequestUrl=${githubPullRequestUrl}`,
    );
