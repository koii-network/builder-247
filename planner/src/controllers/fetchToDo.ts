...
import { getTaskState } from "../utils/taskState";
...
function verifyRequestBody(
  req: Request,
): { signature: string; pubKey: string; github_username: string } | null {
  try {
    console.log("Body:", req.query);
    const signature = req.query.signature as string;
    const pubKey = req.query.pubKey as string;
    const github_username = req.query.github_username as string;
...
async function checkAssignedInfoExists(
  stakingKey: string,
  roundNumber: number,
): Promise<boolean> {
  try {
...
export const fetchTodo = async (req: Request, res: Response) => {
  ...
  console.log()
  if (
    await checkAssignedInfoExists(requestBody.pubKey, signatureData.roundNumber)
  ) {
    res.status(401).json({
      success: false,
