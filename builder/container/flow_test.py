        ...
        if fork_check["success"] and fork_check["exists"]:
            ...
            if not os.path.exists(repo_path):
                ...
                fork_result = fork_repository(
                    f"{self.repo_owner}/{self.repo_name}", self.workspace_dir
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        response = client.send_message(
            "Create or checkout to the branch for repo path './test' only, do not execute the todos:"
            + example_todo
        )
        max_iterations = 10
        ...
        # client.register_tools_from_directory("./src/tools/definitions/github_operations")
        additional_info = (
            "Here is the list of files in the repo: "
            + ", ".join(map(str, files))
            + "Please use cd test to enter the repo path."
        )
        ...
        """
        response = client.send_message(
            "Create a description for the below to do request"
            + example_todo
        ...
        create_pr_result = create_pull_request(
            repo_full_name="HermanKoii/dummyExpress",
            title="feature",
            body=SampleDescription,
            head=f"HermanL02:{branch_name}",
            base="main",
        )
