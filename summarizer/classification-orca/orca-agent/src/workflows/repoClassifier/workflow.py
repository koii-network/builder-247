    ...
    def setup(self):
        try:
            ...
            if not setup_result["success"]:
                raise Exception(
                    f"Failed to set up repository: {setup_result['message']}"
                )
    ...
    def run(self):
        with self.managed_workflow():
            ...
            def retry_classification(phase_class, retries=max_retries):
                for attempt in range(retries):
                    try:
                        ...
                        log_error(
                            e, f"Error during classification attempt {attempt + 1}"
                        )
                        if attempt == retries - 1:
                            raise
                return result
            ...
            try:
                ...
                language_result = retry_classification(
                    phases.LanguageClassificationPhase
                )
                repoMetadata["language"] = extract_value(language_result, "language")

                # Get test framework with retry
                test_framework_result = retry_classification(
                    phases.TestFrameworkClassificationPhase
                )
                repoMetadata["test_framework"] = extract_value(
                    test_framework_result, "test_framework"
                )
                ...
                return {
                    ...
                    "message": (
                        ...
                        if success
                        else "Repository classification failed"
                    ),
                    "data": repoMetadata,
                }
